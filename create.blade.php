...
@section('content')
        <div class="contents">
            <!-- section -------------------------------- -->
            <h2 class="h2">アカウント - 新規登録</h2>
            <section class="section">
                <form name="create_form" action="{{ route('accounts_insert') }}" method="POST" id="form-sys-id">
                    @csrf
                    ...
                    <ul class="form_area">
                        <li>
                            ...
                            <div class="flex_row">
                                <input type="text" name="name" value="{{ old('name') }}" maxlength="64" placeholder="CMSにログインし、案件や人材の管理、提案の登録を行うユーザの名前を登録する">
@if ($errors->has('name'))
...
@foreach ($status as $key => $value)
                                    <li>
                                        <input type="radio" id="status-{{ $key }}" name="status" value="{{$key}}"{{ old('status', config("const.ADMIN_STATUS.using")) == $key ? ' checked' : ''}}>
...
@endif
                        ...
                        <li>
                            ...
                            <div class="flex_row">
                                <input type="email" name="email" value="{{ old('email') }}" maxlength="255" placeholder="メールアドレスを登録する ※他のアカウントで使用しているメールアドレスは登録できません">
@if ($errors->has('email'))
...
@endif
                        ...
                        <li>
                            <label>パスワード<img src="/assets/img/common/required.svg" alt="required"></label>
                            <div class="flex_row">
                                <input type="password" name="password" value="{{ old('password') }}" minlength="8" maxlength="16" placeholder="パスワードは、8文字以上 16文字以下で登録する">
...
@endif
                        ...
                        <li>
                            <label>パスワード(確認)<img src="/assets/img/common/required.svg" alt="required"></label>
                            <div class="flex_row">
                                <input type="password" name="password_confirmation" value="{{ old('password_confirmation') }}" minlength="8" maxlength="16">
...
@endphp
                        <li>
                            ...
                            <account-organization-component
                                ...
                                :error-info="{{ json_encode($errorInfo) }}"></account-organization-component>
                        </li>
...
@foreach ($admin_roles as $admin_role)
                                    <option value="{{ $admin_role->admin_role_id }}" {{( $admin_role->admin_role_id == old('admin_role_id')) ? 'selected' : ''}}>{{ $admin_role->admin_role_name }}</option>
@endforeach
                                </select>
@if ($errors->has('admin_role_id'))
                                <p class="error_text">{{ $errors->first('admin_role_id') }}</p>
@endif
                        ...
                        <li>
                            <label>営業担当コード</label>
                            <div class="flex_row">
                                <input type="number" name="staff_code" value="{{ old('staff_code') }}" min="0" max="4294967295" placeholder="営業担当コードを登録する ※他のアカウントで使用している営業担当コードは登録できません">
...
@endif
                        ...
                        <li>
                            <label>備考</label>
                            <div class="flex_row">
                                <textarea name="note" rows="4" placeholder="備考を登録する" style="width:100%;">{{ old('note') }}</textarea>
@if ($errors->has('note'))
